...
func ObrabotkaNetParser(iLogger logger.ILogger, db *gorm.DB) *parser_services.Parser {
	parser := parser_services.NewParserBuilder().
		Provider().Is(parser_services.ProviderParser{
		Postfix: "companies/?page=",
		...
		CompanyAddressTag: parser_services.NewTagBuilder().
			Build().SetTag(".dl-horizontal dd:nth-child(2)"),
		MailTag: parser_services.NewTagBuilder().
			Build().SetTag(".dl-horizontal dd:nth-child(4) a"),
		CompanyPhoneTag: parser_services.NewTagBuilder().
			Build().SetTag(".company-profile .list-unstyled a"),
		CompanyDescTag: parser_services.NewTagBuilder().
			Build().SetTag("#about p"),
		CompanyServicesTag: parser_services.NewTagBuilder().
			Build().SetTag("#services a"),
